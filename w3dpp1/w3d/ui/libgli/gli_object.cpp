...
void CGLI_Object::SetDimensions(GLfloat width, GLfloat height,
				GLfloat depth)
{				/* {{{ */
    m_width = width;
    m_height = height;
...
GLuint CGLI_Object::SetMouseState(GLuint type, GLuint id, int button,
				  int state)
{				/* {{{ */
    GLuint object_ID = 0;
	...
	if (!((m_event_type == GLI_PRESSED) && (id == m_ID)))
	    m_event_type = GLI_NONE;
	return m_ID;
    } else
	m_event_type = GLI_NONE;
    ...
    if (id == m_ID) {
	if (m_mouse_state == GLI_MOUSE_BUTTON_DOWN)
	    m_event_type = GLI_PRESSED;
	else if (m_mouse_state == GLI_MOUSE_BUTTON_NONE)
	    m_event_type = GLI_ONOVER;
	else {
	    if (m_mouse_state == GLI_MOUSE_BUTTON_UP) {
		if (m_event_type_prev == GLI_PRESSED)
		    m_event_type = GLI_CLICKED;
		else
		    m_event_type = GLI_RELEASED;
	    }
	}
...
void CGLI_Object::
SetCallBackFunction(void (*funct) (GLuint id, GLuint event_type))
{				/* {{{ */
    m_funct = funct;
}				/* }}} */

// Set callback function for this object, (only event_type events are processed)
void CGLI_Object::SetCallBackFunctionByEvent(void (*funct) (GLuint id),
					     GLuint event_type)
{				/* {{{ */
    m_funct_event = funct;
