...
void CGLI_String::TranslatePosition(GLfloat some_constant)
{
	...
	if (m_font_type == GLI_FONT_VECTOR)
	{
		m_pVectorFont->glfGetStringBounds(m_string, &minx, &miny, &maxx, &maxy);
		glTranslatef(0, -(((float)fabs(miny)+(float)fabs(maxy))*m_height + some_constant), 0);
	}
	else
	{
		glTranslatef(0, -(m_pBitmapFont->glfGetMaxHeight()*m_height + some_constant), 0);
	}
}
...
void CGLI_Text::AddNewString(char *string)
{
	...
	{
		cur = m_pStrings;
		while (cur->m_pNext) cur = cur->m_pNext;
		cur->m_pNext = temp;
	}
	temp->m_pNext = NULL;
	temp->SetFontType(m_font_type);
