...
void setup()
{
    ...
    text.
        AddNewString
        (" Usa internamente GLI - Benjamin Schieder - licencia LPGL");
...
//    glOrtho(-100*1.33, 100*1.33, -100, 100, -100, 100);
    glOrtho(-100.0 * (GLfloat) config.SCREEN_SIZE_X /
            (GLfloat) config.SCREEN_SIZE_Y,
            100.0 * (GLfloat) config.SCREEN_SIZE_X /
            (GLfloat) config.SCREEN_SIZE_Y, -100.0, 100.0, -100.0, 100.0);
...
int ui_loop()
{
    ...
    while (done == 0) {
        ...
        if (event.type == SDL_MOUSEMOTION)
            iface.SendMouseParameters(GLI_MOUSE_MOTION,
        ...
        if (event.type == SDL_MOUSEBUTTONDOWN)
            iface.SendMouseParameters(GLI_MOUSE_CLICK,
                                      GLI_MOUSE_BUTTON_LEFT,
                                      GLI_MOUSE_BUTTON_DOWN,
                                      event.button.x,
                                      config.SCREEN_SIZE_Y -
                                      event.button.y);
        if (event.type == SDL_MOUSEBUTTONDOWN)
            iface.SendMouseParameters(GLI_MOUSE_CLICK,
                                      GLI_MOUSE_BUTTON_LEFT,
                                      GLI_MOUSE_BUTTON_UP, event.button.x,
                                      config.SCREEN_SIZE_Y -
                                      event.button.y);
