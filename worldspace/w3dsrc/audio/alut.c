 ...
 * Copyright (C) 1999-2000 by authors --> (www.openal.org)
...
ALvoid alutLoadWAV(ALbyte *file,ALenum *format,ALvoid **data,ALsizei *size,ALsizei *freq, ALboolean *loop)
{
	...
	*loop=AL_FALSE;
    ...
    log_msj("[alut.c] Cargando sonido %s\n", filename);

	if (file)
	{
		Stream=abre_fichero(filename,"rb");
        if (Stream == NULL) {
