...
int carga_fuente(miFuente * prFuente)
{
    ...
    prFuente->mapabits =
	encola_textura(rFic_imagen.sFichero, prFuente->iTipo);
	...
	for (i = 0; i < prFuente->iAncho; i += 16) {
	    ...
	    case FT2:		// Fuente TGA
		prFuente->fCoord[k][0] =
		    (float) i / (float) prFuente->iAncho;
		prFuente->fCoord[k][1] =
		    (float) j / (float) prFuente->iAlto;
		prFuente->fCoord[k][2] =
		    (float) i / (float) prFuente->iAncho +
		    (float) 15 / (float) prFuente->iAncho;
		prFuente->fCoord[k][3] =
		    (float) j / (float) prFuente->iAlto +
		    (float) 15 / (float) prFuente->iAlto;
		break;
	    case AT1:		// Fuente BMP
		prFuente->fCoord[k][0] =
		    (float) i / (float) prFuente->iAncho;
		prFuente->fCoord[k][1] =
		    0.5f - ((float) j / (float) prFuente->iAlto);
		prFuente->fCoord[k][2] =
		    (float) i / (float) prFuente->iAncho +
		    (float) 15 / (float) prFuente->iAncho;
		prFuente->fCoord[k][3] =
	    ...
	    ...)
{
    ...
    for (i = 0; i < iLon; i++) {
	glBegin(GL_QUADS);
	glTexCoord2f(rFuente.fCoord[(text[i] - 32)][0],
		     rFuente.fCoord[(text[i] - 32)][3] + fGrupo);
	glVertex2f(x, y - rFuente.tam_base * tam);
	glTexCoord2f(rFuente.fCoord[(text[i] - 32)][2],
		     rFuente.fCoord[(text[i] - 32)][3] + fGrupo);
	glVertex2f(x + rFuente.tam_base * tam, y - rFuente.tam_base * tam);
	glTexCoord2f(rFuente.fCoord[(text[i] - 32)][2],
		     rFuente.fCoord[(text[i] - 32)][1] + fGrupo);
	glVertex2f(x + rFuente.tam_base * tam, y);
	glTexCoord2f(rFuente.fCoord[(text[i] - 32)][0],
		     rFuente.fCoord[(text[i] - 32)][1] + fGrupo);
	glVertex2f(x, y);
	glEnd();
    ...
    if (rFuente.iTipo == AT1)
	uRecuperarEstado();	// Fuente BMP sin canal Alpha
    return 0;
}
...
//==========================================================================
int imprimeC(miFuente rFuente, int Color, float x, float y, float tam,
	     char *texto, ...)
{
    char text[1024];		// almacena el texto a escribir
    va_list ap;			// puntero a la lista de argumentos (los ...)
